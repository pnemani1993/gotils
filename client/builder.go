...
func NewHttpXBuilder() *HttpXBuilder {
	ctx := context.WithValue(context.Background(), contextKey{}, time.Now().Nanosecond())
	requestContext, err := http.NewRequestWithContext(ctx, "", "", nil)
	if err != nil {
		log.Default()
...
func (builder *HttpXBuilder) WithTimeout(timeout int) *HttpXBuilder {
	builder.timeout = time.Millisecond * time.Duration(timeout)
	return builder
}

func (builder *HttpXBuilder) Build() (*HttpX, error) {
	builder.headers["user-agent"] = []string{"pn-gotils-client"} // sets up the user-agent for the pn-gotils-client - the client
